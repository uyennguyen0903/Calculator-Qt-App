...
Layout::Layout(const QString title, const QString contentName) : title(title), contentName(contentName) {
...
void Layout::addElement(const QString id, const QString content) {
  ...
  if (ids.indexOf(id) == -1) {  // si l'element n'existe pas déjà
    ...
    {
      QHBoxLayout* line = dynamic_cast<QHBoxLayout*>(contentBox->itemAt(i)->widget()->children().at(0));
...
void ProgramLayout::extendModify(const QString edit, const QString label) {
    QStringList list =
            edit.split(QRegExp("\\s+"), Qt::SkipEmptyParts);
    if (list.length() == 0 or list.at(0) != "[") throw ComputerException("Programme invalide.");
...
void VariableLayout::extendModify(const QString edit, const QString label) {
    ...
    if (type != Operand::OperandType::kInteger and type != Operand::OperandType::kReal and type != Operand::OperandType::kFraction)
